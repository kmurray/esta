...
class HoldAnalysisMode : public BaseAnalysisMode {
    public:
        ...
        //External tag access
        const Tags& hold_data_tags(NodeId node_id) const { return hold_data_tags_[node_id]; }
        const Tags& hold_clock_tags(NodeId node_id) const { return hold_clock_tags_[node_id]; }
    protected:
        ...
        std::vector<Tags> hold_data_tags_; //Data tags for each node [0..timing_graph.num_nodes()-1]
        std::vector<Tags> hold_clock_tags_; //Clock tags for each node [0..timing_graph.num_nodes()-1]
};
